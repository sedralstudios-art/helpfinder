export const COLLECTING_COURT_JUDGMENT_NY = {
  ...
  whatItMeans: {
    en: "When a court orders someone to pay money, the person who won (called the judgment creditor) has the legal right to collect. But the court does not collect for you. If the person who owes (called the judgment debtor) does not pay on their own, the creditor can use several tools. An information subpoena is a form that forces the debtor to answer questions about their income, bank accounts, and property. A restraining notice freezes the debtor's bank account. A property execution allows a marshal or sheriff to seize money from the bank account. An income execution takes a portion of the debtor's wages — up to 10% of gross income or 25% of disposable income, whichever is less. These tools are filed through the county clerk or the court that issued the judgment. A judgment in New York is good for 20 years. Interest runs at 9% per year from the date of the judgment. If the debtor has no money, no job, and no assets, the judgment may be uncollectable for now — but it remains valid and can be enforced later when the debtor's situation changes."
  },
